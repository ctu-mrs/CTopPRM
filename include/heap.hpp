...
template <typename T> struct HeapNode {
  ...
  std::unordered_map<HeapNode<T> *, double> visibility_node_ids;
  std::multimap<double, HeapNode<T> *> dist_sorted_visibility_node_ids;
...
template <class T> void Heap<T>::clear() { heapVector.clear(); }

template <class T> bool Heap<T>::empty() { return this->heapVector.empty(); }

template <class T> std::vector<T> Heap<T>::getHeapVector() {
  return this->heapVector;
}

template <class T> bool Heap<T>::checkOrdering() { return checkIndex(0); }
